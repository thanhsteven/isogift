...
if ( ! function_exists( 'woodmart_product_sale_countdown' ) ) {
	function woodmart_product_sale_countdown( $settings = array() ) {
		...
		if ( ( ! Builder::get_instance()->has_custom_layout( 'single_product' ) && ( 'wpb' === woodmart_get_current_page_builder() || ! woodmart_elementor_is_edit_mode() ) && is_single() && empty( $settings['timer_style'] ) ) || ( ! empty( $settings['products_hover'] ) && in_array( $settings['products_hover'], array( 'info', 'info-alt', 'alt', 'icons', 'quick', 'button', 'standard' ), true ) ) ) {
			$wrapper_classes .= ' timer-style-standard';
		}

		if ( ! empty( $settings['timer_style'] ) ) {
			$wrapper_classes .= ' timer-style-' . $settings['timer_style'];
		}
...
if ( ! function_exists( 'woodmart_woo_wrapper_start' ) ) {
	function woodmart_woo_wrapper_start() {
		...
		if ( is_singular( 'product' ) ) {
			...
			if ( ! Builder::get_instance()->has_custom_layout( 'single_product' ) ) {
				if ( 'default' === $breadcrumbs_position && 'default' === $product_design ) {
					$breadcrumbs_position = 'summary';
				} elseif ( 'default' === $breadcrumbs_position && 'alt' === $product_design ) {
		...
		$content_class .= woodmart_get_builder_status_class();
...
if ( ! function_exists( 'woodmart_filter_buttons' ) ) {
	function woodmart_filter_buttons() {
		...
		?>
			<div class="wd-filter-buttons wd-action-btn wd-style-text<?php echo woodmart_get_old_classes( ' woodmart-filter-buttons' ); ?>">
...
if ( ! function_exists( 'woodmart_sticky_sidebar_button' ) ) {
	function woodmart_sticky_sidebar_button( $echo = true, $toolbar = false ) {
		...
		if ( ( strstr( $sidebar_class, 'col-lg-0' ) && ! $is_builder ) || woodmart_maintenance_page() || is_singular( 'woodmart_slide' ) || is_singular( 'cms_block' ) || ( $sticky_toolbar && in_array( 'sidebar', $sticky_toolbar_fields ) && ! $toolbar ) || woodmart_is_elementor_full_width( true ) ) {
			return;
		}
		...
		$is_singular = ( ! woodmart_woocommerce_installed() || ( ! is_shop() && ! is_product_category() && ! is_product_tag() && ! is_product_taxonomy() && ! is_404() ) ) && ! is_singular( 'portfolio' ) && ! woodmart_is_portfolio_archive();
		?>
		...
		<?php if ( $is_shop && ( ( ! Builder::get_instance()->has_custom_layout( 'shop_archive' ) && $sticky_filter_button && ( woodmart_get_opt( 'shop_hide_sidebar' ) || woodmart_get_opt( 'shop_hide_sidebar_tablet' ) ) ) || ( Builder::get_instance()->has_custom_layout( 'shop_archive' ) && ! empty( Builder_Data::get_instance()->get_data( 'wd_show_sticky_sidebar_button' ) ) && $toolbar ) ) ) : ?>
			...
			<div class="wd-sidebar-opener wd-on-shop wd-filter-icon<?php echo esc_attr( $classes ); ?>">
				<a href="#" rel="nofollow">
		...
		<?php elseif ( ( woodmart_get_opt( 'hide_main_sidebar_mobile' ) && ! Builder::get_instance()->has_custom_layout( 'single_product' ) && $is_singular ) || Builder::get_instance()->has_custom_layout( 'single_product' ) && ! empty( Builder_Data::get_instance()->get_data( 'wd_show_sticky_sidebar_button' ) ) && $toolbar ) : ?>
			<?php woodmart_enqueue_inline_style( 'mod-sticky-sidebar-opener' ); ?>

			<div class="wd-sidebar-opener wd-not-on-shop<?php echo esc_attr( $classes ); ?>">
				<a href="#" rel="nofollow">
					<span class="<?php echo esc_attr( $label_classes ); ?>">
